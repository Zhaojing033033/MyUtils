...
public class CRC16Util {
    ...
    /**
     * 将计算的CRC值 补足4位，并高低位互换  比如  ： crc值为 A30A -> A3 0A
     * @param res
     * @return
     */
    public static String getCrc(int res) {
        String format = String.format("%04x", res);
        String height = format.substring(0, 2);
        String low = format.substring(2, 4);
        return low.concat(height).toUpperCase();
    }
    /**
     * 将报文进行crc检查，并将检查结果拼接到报文后面，返回
     */
    public static String crcDeal(String data){
        byte[] checkConent = ByteUtil.hexStringToBytes(data);
        int crcCheck = CRC16Util.calcCrc16(checkConent);
        String crcCheckHexStr = CRC16Util.getCrc(crcCheck);
        return data+crcCheckHexStr;
    }

    public static void main(String[] args) {
        byte[] checkConent = ByteUtil.hexStringToBytes("010300000002");//C40B
        int crcCheck = CRC16Util.calcCrc16(checkConent);
        //正确结果应该为 9bab
        String crcCheckHexStr = CRC16Util.getCrc(crcCheck);
